...
class Solver:
    ...
    def find_solutions(self, ants, world):
        ...
        while ants_done < len(ants):
            ...
            for ant in ants:
                if ant.can_move():
                    edge = ant.move()
                    self.local_update(edge, world)
                else:
                    ants_done += 1

    def local_update(self, edge, world):
        ...
        """
        if edge != world.dummy_edge and edge != None:
            edge.pheromone = max(self.t0, edge.pheromone * self.rho)

    def global_update(self, ants, world):
