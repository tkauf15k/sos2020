...
class aco(algorithm_base):
    ...
    def solve(self, num_sequences, time_limit):
        ...
        def improvement_callback(fitness, iteration, solution):
            matching = [(solution[i], solution[i + 1]) for i in range(0, len(solution) - 1, 2)]
        ...
        def local_search_callback(tour):
            ...
            improved_matching, fitness = localsearch(matching, cost_matrix, num_iterations=20, first_improvement=False)
        ...
        solver = pants.Solver(ant_count=100, limit=time_limit, optimum=optimum, improvement_callback=improvement_callback,
